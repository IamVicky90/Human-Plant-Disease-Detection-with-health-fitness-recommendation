...
def pred_pnemoian(img_path):
  log=add_logger()
  try:
    model_path=os.path.join("Model","Chest XRay Pnemonia xception model.h5")
    model=load_model(model_path)
    log.log(f'sucessfully read the model in path {str(model_path)}','predict_images.log',1)  
  except Exception as e:
    try:
        log.log(f'Could not read the model in path {str(model_path)} error {str(e)}','predict_images.log',3) 
    except Exception as NameError:
        log.log(f'{str(NameError)} occured while reading the model path error {str(e)}','predict_images.log',3) 
  try:
    ...
    log.log(f'kidney_disease_pred funtion run sucessfully','predict_images.log',1)    
  except Exception as e:
      log.log(f'Could not run pred_pnemoian funtion sucessfully error {str(e)}','predict_images.log',3)
def pred_plant_dieas(plant):
  log=add_logger()
  try:
    model_path=os.path.join("Model","Various Plant Disease Detection Model1.h5")
    model=load_model(model_path)
    log.log(f'sucessfully read the model in path {str(model_path)}','predict_images.log',1)  
  except Exception as e:
    try:
...
def pred_skin(img_path):
  log=add_logger()
  try:
    model_path=os.path.join("Model","skin cancer vgg16 model.h5")
    model=load_model(model_path)
    log.log(f'sucessfully read the model in path {str(model_path)}','predict_images.log',1)  
  except Exception as e:
    try:
  ...
  try:
    ...
    log.log(f'Could not run pred_skin funtion sucessfully error {str(e)}','predict_images.log',3)